...
def login_access_token(user, password):
    ...
    print(f"  加密前数据: {query}")
    print(f"  加密后数据: {cipher_data}")
    
    r = requests.post(url, data=cipher_data, headers=headers, allow_redirects=False, timeout=10)
    
    print(f"  登录响应状态码: {r.status_code}")
    print(f"  响应头: {dict(r.headers)}")
    print(f"  响应内容(前200字符): {r.text[:200] if r.text else '空'}")
    
    if r.status_code != 303:
        return None, f"登录异常，status: {r.status_code}"
    ...
    try:
        ...
        if result is None or len(result) == 0:
            ...
            error_result = error_pattern.findall(location)
            if error_result:
                return None, f"登录失败，错误码: {error_result[0]}"
            return None, "获取accessToken失败，未找到token"
        
        access_token = result[0]
        print(f"  ✓ 获取到access_token: {access_token[:30]}...")
        return access_token, None
    except Exception as e:
        return None, f"获取accessToken异常: {str(e)}"

def grant_login_tokens(access_token, device_id, is_phone=False):
    """步骤2: 使用access_token获取login_token, app_token, user_id"""
    ...
    if resp.get("result") != "ok":
        return None, None, None, f"客户端登录失败: {resp.get('result')}"
    
    login_token = resp["token_info"]["login_token"]
    app_token = resp["token_info"]["app_token"]
    user_id = resp["token_info"]["user_id"]
...
class ZeppLifeStepPusher:
    """通过Zepp Life(小米运动)增加微信步数"""
    ...
    def login(self):
        """登录Zepp Life账号"""
        login_url = "https://api-user.huami.com/authservice"
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "MiFit/4.6.8"
        }
        login_data = {
            "client_id": "HuaMi",
            "country": "CN",
            "device_id": "2C8B4A9E-XXXX-XXXX-XXXX-XXXXXXXXXXXX",
        ...
        if result.get("code") == 0:
            self.token = result["token_info"]["access_token"]
    ...
    def get_app_token(self):
        """获取应用token"""
        url = "https://api-user.huami.com/authservice"
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "MiFit/4.6.8"
        }
        ...
        if result.get("code") == 0:
            self.app_token = result["token_info"]["app_token"]
            return True
        return False
    
    def upload_steps(self, date, steps):
        """上传步数到Zepp Life"""
        if not self.app_token:
            return False
        
        url = "https://sportswww.huami.com/v1/footprint"
        headers = {
            "Content-Type": "application/json",
    ...
    def push_steps(self, phone, password, date, steps):
        """推送步数的完整流程"""
        self.phone = phone
        self.password = password
        
        print(f"正在登录账号: {self.phone}")
        if not self.login():
            print("登录失败")
            return False
        
        print("登录成功，获取App Token")
        if not self.get_app_token():
            print("获取App Token失败")
            return False
        
        print(f"正在上传步数: {steps} (日期: {date})")
        if self.upload_steps(date, steps):
            print("步数上传成功!")
            return True
        else:
            print("步数上传失败")
...
def push_steps_interactive():
    """交互式步数上传"""
    import string
    
    print("=" * 60)
    print("Zepp Life 步数上传")
    print("=" * 60)
    
    user_input = input("请输入手机号: ").strip()
    ...
    device_id = "2C8B4A9E-XXXX-XXXX-XXXX-" + ''.join(random.choices(string.hexdigits.upper(), k=12))
    
    print(f"\n[1] 获取access_token...")
    
    access_token, error = login_access_token(phone, password)
    ...
    login_token, app_token, user_id, error = grant_login_tokens(access_token, device_id, is_phone)
    ...
    target_steps = input("请输入目标步数 (直接回车使用35000): ").strip()
    if not target_steps:
        target_steps = "35000"
    
    if not target_steps.isdigit():
        print("步数必须是数字!")
        return
    
    target_steps = int(target_steps)
    
    if target_steps < 0 or target_steps > 999999:
        print("步数必须在 0 - 999999 之间!")
        return
    
    print(f"\n[3] 上传步数: {target_steps}")
    
    success, message = post_fake_brand_data(str(target_steps), app_token, user_id)
...
def main():
    ...
    push_steps_interactive()
    
    print("\n如需正式使用，请取消注释并配置真实账号")
    # pusher = ZeppLifeStepPusher()
    # phone = "你的手机号"
    # password = "你的密码"
    # target_date = datetime.now().strftime("%Y-%m-%d")
    # target_steps = 25000
    # success = pusher.push_steps(phone, password, target_date, target_steps)
    # print(f"步数上传结果: {success}")
